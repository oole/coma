...
from model.spiral_layers import encoder_block, decoder_block, spiral_conv
from tensorflow import keras


class coma_spiral_ae(keras.Model):
    """
    ...
    def __init__(self,
                 ...
                 regularization,
                 **kwargs):
        super(coma_spiral_ae, self).__init__(**kwargs)
...
class encoder(keras.Model):
    ...
    def __init__(self,
        ...
        for i in range(len(num_features)):
            if i == 0:
                self.encoder_blocks.append(encoder_block(spirals=spirals[i],
...
class decoder(keras.Model):
    ...
    def __init__(self,
        ...
        for i in range(len(num_features)):
            if i >= 1:
                self.decoder_blocks.append(decoder_block(spirals=spirals[len(num_features) - i - 1],
                ...
                self.decoder_blocks.append(decoder_block(spirals=spirals[len(num_features)-i - 1],
                                                         ...
                                                         upsampling_transformation=upsampling_transformations[-i - 1],
                                                         batch_size=batch_size))
        self.decoder_output = spiral_conv(
