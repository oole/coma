...
parser.add_argument("--prediction-spiral", default="spiral-coma-model/results/spiral_lr8e3_sliced_coma_2201100844_result.npy",
                    help="The path to the predictions.npy")
parser.add_argument("--prediction-cheb", default="results/lr8e3_sliced_coma_result.npy",
                    help="The path to the predictions.npy")
parser.add_argument("--data-dir", default="data/sliced",
                    help="The path to the data")
parser.add_argument("--error-dir", default="spiral-coma-model/error-vs", help="The directory where the computed errors should be stored")
...
def plot_error_over_vertices(cheb_coma_error, spiral_coma_error, pca_error, vertices, path):
    ...
    plt.plot(x_cheb_coma, y_cheb_coma, label="CoMA C")
    plt.plot(x_spiral_coma, y_spiral_coma, label="CoMA S")
...
predictions_cheb = np.load(args.prediction_cheb)
date_print("Calculating errors for " + args.prediction_cheb + " - Dir: " + args.data_dir)
...
predicted_cheb_vertices = (predictions_cheb * mesh_data.std) + mesh_data.mean

# CoMA spiral
predictions_spiral = np.load(args.prediction_spiral)
predicted_spiral_vertices = (predictions_spiral * mesh_data.std) + mesh_data.mean

# Original
original_vertices = (mesh_data.vertices_test[:predictions_cheb.shape[0]] * mesh_data.std) + mesh_data.mean
...
# CoMA cheb
cheb_model_error, cheb_model_error_mean, cheb_model_error_std, cheb_model_error_median = calculate_error(
    predicted_cheb_vertices_mm,
    original_vertices_mm)

date_print(
    "Cheb CoMA Error - Mean: " + str(cheb_model_error_mean) + ", Std: " + str(
        cheb_model_error_std) + ", Median: " + str(
        cheb_model_error_median))

# CoMA spiral
spiral_model_error, spiral_model_error_mean, spiral_model_error_std, spiral_model_error_median = calculate_error(
    predicted_spiral_vertices_mm,
    original_vertices_mm)

date_print(
    "Spiral CoMA Error - Mean: " + str(spiral_model_error_mean) + ", Std: " + str(
        spiral_model_error_std) + ", Median: " + str(
        spiral_model_error_median))
...
plot_error_over_vertices(cheb_model_error, spiral_model_error, pca_error, original_vertices,
...
# Cheb CoMA
cheb_coma_error_file = args.error_dir + "/" + "cheb_coma_" + os.path.basename(args.data_dir)
with open(cheb_coma_error_file + ".json", 'w') as file:
    save_params = dict()
    # save_params['coma_model_error'] = model_error
    save_params['cheb_coma_model_error_mean'] = cheb_model_error_mean
    save_params['cheb_coma_model_error_std'] = cheb_model_error_std
    save_params['cheb_coma_model_error_median'] = cheb_model_error_median
    date_print(str(save_params))
    json.dump(save_params, file)
np.save(cheb_coma_error_file + "_error.npy", cheb_model_error)

# Spiral CoMA
spiral_coma_error_file = args.error_dir + "/" + "spiral_coma_" + os.path.basename(args.data_dir)
with open(spiral_coma_error_file + ".json", 'w') as file:
    save_params = dict()
    # save_params['coma_model_error'] = model_error
    save_params['spiral_coma_model_error_mean'] = spiral_model_error_mean
    save_params['spiral_coma_model_error_std'] = spiral_model_error_std
    save_params['spiral_coma_model_error_median'] = spiral_model_error_median
    date_print(str(save_params))
    json.dump(save_params, file)
np.save(spiral_coma_error_file + "_error.npy", spiral_model_error)
